...
export const ResaleBenefitsSection = () => {
  return (
    <AnimatedSection id="revenda" className="relative overflow-hidden bg-[#0b0d0e] text-white">
      <XingyuBackground variant="section" imageSrc="/bg1.png" imageOpacity={0.18}className="opacity-70" />
            ...
            {benefitItems.map(({ title, description, Icon }) => (
              ...
              >
                ...
                <div className="min-w-0">
                  <h3 className="text-[0.98rem] font-medium tracking-tight text-white md:text-[1.05rem]">{title}</h3>
                  <p className="mt-1.5 text-sm leading-relaxed text-zinc-300 md:text-[0.95rem]">{description}</p>
                </div>
              </motion.li>
            ))}
          </motion.ul>
