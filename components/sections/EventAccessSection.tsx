...
export const EventAccessSection = () => {
  return (
    <Section id="access" className="relative overflow-hidden bg-[#0b0d0e] text-white">
      <XingyuBackground variant="section" imageSrc="/background2.png" imageOpacity={0.16} className="opacity-60" />
      ...
      <Container className="space-y-14">
        ...
        >
          <div className="inline-flex items-center justify-center gap-4 sm:justify-start">
            ...
            <span className="text-[0.62rem] uppercase tracking-[0.42em] text-zinc-300">Acesso Exclusivo</span>
          </div>
          <h2 className="mt-5 text-4xl font-light leading-[1.05] tracking-tight text-white">
            O que você terá acesso neste <span className="font-medium">evento</span>?
          </h2>
        </motion.div>
        ...
        <div className="grid gap-7 md:grid-cols-3 md:gap-8">
          {accessItems.map(({ title, image, glow }) => (
            ...
            >
              <div aria-hidden="true" className={`absolute -right-32 -top-32 h-96 w-96 rounded-full bg-gradient-to-br ${glow} blur-3xl opacity-25`} />
              <div aria-hidden="true" className="absolute inset-0 bg-[radial-gradient(circle_at_15%_10%,rgba(255,255,255,0.14),transparent_55%)]" />
              <Image
                src={image}
                alt={title}
                fill
              ...
              <div className="relative flex min-h-[19.5rem] flex-col justify-between p-7 md:p-8">
                ...
                <div>
                  <p className="max-w-[20rem] text-[1.15rem] md:text-[1.22rem] leading-snug text-white/95 font-light tracking-tight">
                    {title}
                  </p>
                  <div aria-hidden="true" className="mt-7 h-px w-full bg-gradient-to-r from-transparent via-white/18 to-transparent" />
                </div>
              </div>
            </motion.div>
          ))}
